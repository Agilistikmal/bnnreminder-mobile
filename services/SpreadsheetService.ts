...
export function calculateKGBStatus(kgbData: KGBData): 'akan_datang' | 'waktunya' | 'terlambat' {
    const today = new Date();
    const kgbDate = parseIndonesianDate(kgbData.kgbBerikutnya);

    // Konversi ke timestamp untuk perbandingan
    const diffDays = Math.floor((kgbDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
